...
export const ActionBar = () => {
    ...
    return (
        <div className="flex w-full gap-2 items-center justify-center my-3 px-2 py-4 bg-base-content/30 rounded-2xl" >
            ...
            <div className="breadcrumbs overflow-x-visible flex-1 w-full flex items-center bg-base-content/50 px-2 py-1 rounded-2xl ">
                <ul>
                    {(location?.split("/"))?.map((path,key)=>(
                        <li key={key}>
                            <Link href={"/fileManager?location=" + path } onClick={()=> setIncludeInHistory(true)}>{path}</Link>
                        </li>
                    ))}
                </ul>
            </div>
