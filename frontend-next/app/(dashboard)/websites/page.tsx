...
import { getWebsiteList } from "../actions/getWebsites"
...
export default function Websites() {
    ...
    const [websiteLst, setWebsiteList] = useState<WebsiteInfo[]>([])
    const [selectMode, setSelectMode] = useState(false)
    useEffectOnce(() => {
        const loadList = async () => {
            const tempList = await getWebsiteList()
            if (tempList) setWebsiteList(tempList)
            else { }
        }
        loadList()
    })
                ...
                <div className="flex flex-col items-center w-full h-full gap-4 md:gap-5  overflow-y-auto">
                    {websiteLst.map((info, key) => {
                        if (selectMode) {
                            ...
                            return (
                                <Link href={"/websites"} className="btn btn-ghost btn-lg flex-nowrap flex justify-center md:space-x-4 items-center w-full px-3 md:px-7 py-[2.125rem] md:py-10 bg-base-content/5 hover:bg-base-content/20 cursor-pointer transition duration-200 rounded-lg text-[15px] md:text-base" key={key}>
